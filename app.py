...
async def add(data):
    conn = await asyncpg.connect('postgresql://postgres:postgres@127.0.0.1:5002/swapi_db')
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        for chunk_id in chunked(all_ids, PARTITION):
            ...
            transform_coros = [transform(char) for char in chars]
            transformed_chars = await asyncio.gather(*transform_coros)
            db_coros = [add(char) for char in transformed_chars]
